...
def get_bin_counts(array_in: np.ndarray, num_bins_per_dim: int) -> np.ndarray:
    """Deprecated: use compute_bin_counts instead."""
    return compute_bin_counts(array_in, num_bins_per_dim)


def get_bin_vals(num_bins: int) -> Array:
    """Deprecated: use make_grid_indices instead."""
    return make_grid_indices(num_bins)


def gram(kernel: Callable, xs: Array) -> Array:
    """Deprecated: use compute_gram_matrix instead."""
    return compute_gram_matrix(kernel, xs)
...
def kernel_func(
    ...
    """Deprecated: use exponential_kernel instead."""
    return exponential_kernel(x, y, signal_variance, num_grid_per_dim, raw_length_scale)


def poisson_process_log_likelihood(
    latent_function: Array,
    bin_area: float,
    flat_bin_counts: Array,
) -> Array:
    """Deprecated: use poisson_log_likelihood instead."""
    return poisson_log_likelihood(latent_function, bin_area, flat_bin_counts)
...
def get_latents_from_white(
    ...
    """Deprecated: use whiten_to_latent instead."""
    return whiten_to_latent(white, const_mean, cholesky_gram)
...
def get_white_from_latents(
    ...
    """Deprecated: use latent_to_whiten instead."""
    return latent_to_whiten(latents, const_mean, cholesky_gram)
